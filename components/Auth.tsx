...
import { useFMS, DEFAULT_CLEAN_STATE } from '../context/FMSContext';
...
const Auth: React.FC<AuthProps> = ({ mode: initialMode, onNavigate }) => {
  ...
  // Contexts
  const { dispatch, refreshFromApi } = useFMS();
  ...
  const handleImmediateDemoLogin = async (roleToUse: 'Admin' | 'User') => {
    ...
    const roleParam = roleToUse === 'Admin' ? 'admin' : 'user';
    ...
    try {
      // Call Laravel Backend API for Demo Login with role
      const res = await authApi.demoLogin(roleParam);
      if (res && res.success) {
        localStorage.setItem('fms_auth_token', res.token);
        localStorage.setItem('fms_active_user_email', res.user?.email || targetEmail);
        
        const initialCleanState = {
          ...DEFAULT_CLEAN_STATE,
          role: roleToUse,
          subscription: (res.user?.is_pro || roleToUse === 'Admin' ? 'Pro' : 'Free') as 'Free' | 'Pro',
          currentUserEmail: res.user?.email || targetEmail,
          activeUser: res.user,
          users: [
            { id: String(res.user?.id || (roleToUse === 'Admin' ? '1' : '2')), name: res.user?.name || targetName, email: res.user?.email || targetEmail, role: roleToUse, subscription: (res.user?.is_pro || roleToUse === 'Admin') ? 'Pro Plan' : 'Free Plan' }
          ]
        };

        dispatch({
          type: 'LOGIN_USER',
          payload: { email: res.user?.email || targetEmail, stateData: initialCleanState, token: res.token }
        });

        // Pull full authentic database records for transactions, coa, assets, users
      ...
      const msg = apiErr.response?.data?.message || (language === 'id' ? 'Gagal terhubung ke server demo. Pastikan API backend aktif.' : 'Failed to connect to demo server.');
      setError(msg);
      setIsLoading(false);
    }
  };
    ...
    if (mode === 'register') {
      try {
        ...
        if (res && res.success) {
          ...
          const freshNewState = {
            ...
            role: res.user.role ? (res.user.role === 'admin' ? 'Admin' : 'User') : 'User',
        ...
        if (err.response && err.response.data && err.response.data.errors) {
          ...
          setError(language === 'id' ? 'Gagal mendaftar ke server API. Periksa koneksi backend.' : 'Registration failed. Check backend connection.');
        }
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        if (res && res.success) {
          ...
          const determinedRole: 'Admin' | 'User' = (res.user.role === 'admin' || res.user.role === 'Admin' || res.user.role === 'demo') ? 'Admin' : 'User';
          ...
          const targetStateData = {
            ...
            activeUser: res.user,
            users: [
              { id: String(res.user.id), name: res.user.name, email: normalEmail, role: determinedRole, subscription: res.user.is_pro ? 'Pro Plan' : 'Free Plan', status: 'Active' }
            ]
          };
          ...
          // Fetch full database transactions, assets, coa, etc.
          await refreshFromApi();
        ...
        if (err.response && err.response.data && err.response.data.message) {
          ...
          setError(language === 'id' ? 'Koneksi ke backend gagal atau kata sandi tidak cocok.' : 'Login failed. Invalid credentials or server error.');
        }
        setIsLoading(false);
        return;
      }
