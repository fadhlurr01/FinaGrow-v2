...
const Assets: React.FC = () => {
  ...
  // Use authentic assets from FMSContext state directly from DB
  const assets: Asset[] = useMemo(() => {
    const rawAssets = Array.isArray(state.assets) ? state.assets : [];
    
    return rawAssets.map(a => {
      const rawCost = (a as any).purchaseCost ?? (a as any).purchase_cost ?? 0;
      const parsedCost = Number(rawCost) || 0;
      const parsedDate = (a as any).purchaseDate ?? (a as any).purchase_date ?? new Date().toISOString().slice(0, 10);
      const parsedLife = Number((a as any).usefulLife ?? (a as any).useful_life ?? 1);
      const parsedMethod = (a as any).depreciationMethod ?? (a as any).depreciation_method ?? 'Straight Line';

      return {
        id: String(a.id),
        code: a.code || `AST-${a.id}`,
        name: a.name || 'Unnamed Asset',
        category: (a.category as any) || 'Equipment',
        purchaseCost: parsedCost,
        purchaseDate: parsedDate,
        usefulLife: parsedLife,
        depreciationMethod: parsedMethod
      };
    });
  }, [state.assets]);
