...
const Header: React.FC<HeaderProps> = ({ currentView, isMobileSidebarOpen, setIsMobileSidebarOpen, onLogout }) => {
  ...
  const { state, dispatch, refreshFromApi, upgradeToProApi, isLoading } = useFMS();
  ...
  // Determine user identity dynamically from backend state
  const activeUser = state.activeUser || state.users?.find((u: any) => u.email === state.currentUserEmail);
  const activeEmail = state.currentUserEmail;
  const userName = activeUser?.name || (state.role === 'User' ? 'Demo User' : 'Demo Admin');
  const userEmail = activeEmail || 'demo_admin@fms.com';
  const userPlan = (state.subscription === 'Pro' || state.role === 'Admin') ? 'Pro Plan' : 'Free Plan';
  
  // Load avatar image dynamically from backend user profile or fallback
  const userAvatar = activeUser?.avatar || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=100';
    ...
    <header className="flex items-center justify-between h-20 px-6 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800/40 z-20 sticky top-0">
      ...
      <div className="flex items-center gap-1.5 sm:gap-4">
        ...
        <button
          type="button"
          onClick={async () => {
            await upgradeToProApi(isPro ? 'Free' : 'Pro');
          }}
