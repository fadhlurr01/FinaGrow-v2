...
const Users: React.FC = () => {
  ...
  // Fetch live users directly from backend API
  ...
  // Load from central database repository or context
  const users: User[] = useMemo(() => {
    ...
    if (state.users && state.users.length > 0) {
      return state.users.map((u: any) => ({
        ...
        email: u.email,
        role: u.role || 'User',
        subscription: u.subscription || (u.is_pro ? 'Pro Plan' : 'Free Plan'),
        status: u.status || (u.is_banned ? 'Suspended' : 'Active')
      }));
    }
    return [];
