...
const Profile: React.FC = () => {
  ...
  const activeUser = state.activeUser || state.users?.find((u: any) => u.email === state.currentUserEmail) || fallbackUser;

  const [name, setName] = useState(activeUser.name || '');
  const [phone, setPhone] = useState(activeUser.phone || '08123456789');
  const [successMsg, setSuccessMsg] = useState('');

  // State for avatar image from database
  const [avatar, setAvatar] = useState<string>(activeUser.avatar || '');
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        ...
        try {
          ...
          if (res && res.user) {
            dispatch({ type: 'SET_ACTIVE_USER', payload: res.user });
          }
          setSuccessMsg(language === 'id' ? 'Foto profil berhasil disimpan ke database!' : 'Profile picture saved to database!');
        } catch (err) {
  ...
  const [department, setDepartment] = useState(activeUser.department || 'Finance & Treasury');
  const [jobTitle, setJobTitle] = useState(activeUser.job_title || (state.role === 'Admin' ? 'Senior Financial Controller' : 'Associate Analyst'));
  const [costCenter, setCostCenter] = useState(activeUser.cost_center || 'CC-ID-JAB-204');
  const [employeeId, setEmployeeId] = useState(activeUser.employee_id || 'EMP-98839211');

  useEffect(() => {
    if (state.activeUser) {
      setName(state.activeUser.name || '');
      setPhone(state.activeUser.phone || '');
      setAvatar(state.activeUser.avatar || '');
      if (state.activeUser.department) setDepartment(state.activeUser.department);
      if (state.activeUser.job_title) setJobTitle(state.activeUser.job_title);
      if (state.activeUser.cost_center) setCostCenter(state.activeUser.cost_center);
      if (state.activeUser.employee_id) setEmployeeId(state.activeUser.employee_id);
    }
  }, [state.activeUser, state.role]);
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res && res.success) {
        setPassSuccessMsg(language === 'id' ? 'Kata sandi Anda berhasil diperbarui di database!' : 'Your password has been successfully updated!');
        setCurrentPassword('');
        setNewPassword('');
        setConfirmNewPassword('');
      }
    } catch (err: any) {
      const msg = err.response?.data?.message || (language === 'id' ? 'Kata sandi lama Anda tidak cocok atau terjadi kesalahan sistem!' : 'Old password does not match or system error occurred!');
      setPassErrorMsg(msg);
    }
  };
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res && res.user) {
        ...
        setSuccessMsg(language === 'id' ? 'Profil Anda berhasil disimpan ke database!' : 'Your profile has been successfully updated!');
        setTimeout(() => setSuccessMsg(''), 4000);
      }
    } catch (err: any) {
      const msg = err.response?.data?.message || (language === 'id' ? 'Gagal menyimpan profil ke database.' : 'Failed to save profile.');
      setSuccessMsg(msg);
    }
  };
