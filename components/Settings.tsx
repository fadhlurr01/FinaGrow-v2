...
const Settings: React.FC = () => {
  const { state, dispatch, refreshFromApi } = useFMS();
  ...
  const handleRestoreData = async () => {
    try {
      ...
      triggerToast(
        language === 'id' 
          ? 'Data berhasil disinkronkan langsung dari database server!' 
          : 'Data successfully synchronized directly from database server!'
      );
    } catch (e) {
      triggerToast(
        language === 'id' 
          ? 'Gagal menyinkronkan data dari database server.' 
          : 'Failed to synchronize data from database server.'
      );
    }
  };
