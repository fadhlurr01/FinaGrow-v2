...
const LayoutContainer: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Check ban state from backend authenticated user
  const currentUser = state.activeUser || state.users?.find((u: any) => u.email === state.currentUserEmail);
  const isBanned = !!(currentUser && (currentUser.status === 'Banned' || currentUser.status === 'Suspended' || currentUser.is_banned || currentUser.isBanned));
